  ...
  return (
    <div className="card p-4 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-[#64748B] uppercase tracking-wider font-medium">{title}</span>
      </div>
      <p className="text-xl font-bold text-white">{value}</p>
      {sub && <p className="text-[11px] text-[#475569]">{sub}</p>}
    </div>
  )
}
...
export default function SkyConditionsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 animate-page-enter flex flex-col gap-4">
      {/* Header */}
      <header className="flex items-start justify-between">
        <div>
          <span className="text-[10px] font-bold tracking-[0.15em] text-[#64748B] uppercase block mb-1">
            {lang === 'ka' ? 'სადგური' : 'Observatory'}
          </span>
          <h1 className="text-xl sm:text-2xl font-bold text-white">{t('sky.title')}</h1>
          <p className="text-sm text-[#64748B] mt-0.5">{t('sky.subtitle')}</p>
        </div>
        {lastUpdate && (
          <p className="text-[10px] text-[#475569] text-right mt-1 font-mono">{t('sky.updated')} {lastUpdate}</p>
        )}
      </header>

      {loading && !weather && (
        <div className="card p-8 text-center text-[#64748B] text-sm">{t('sky.loading')}</div>
      )}

      {weather && score && (
        <>
          {/* Sky Score */}
          <div className="card p-5 flex items-center gap-6" style={{ borderColor: score.color + '40' }}>
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1">
                {[
                  ...
                  <span key={b.label} className="text-[10px] text-[#475569]">{b.label} <span className="text-red-400">{b.val}</span></span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="card p-5">
            <p className="text-xs text-[#64748B] uppercase tracking-widest mb-4 font-medium">{t('sky.moonProgress')}</p>
          ...
          {weather.hourly.length > 0 && (
            <div className="card p-5">
              <p className="text-xs text-[#64748B] uppercase tracking-widest mb-4 font-medium">
                ...
                {weather.hourly.map(h => {
                  ...
                  return (
                    <div key={h.timeLabel} className="flex-1 flex flex-col items-center gap-1">
                      <span className="text-[8px] text-[#475569]">{pct}%</span>
                      <div className="w-full rounded-sm" style={{ height: `${barH}px`, background: color + 'cc', minWidth: 6 }} />
                      <span className="text-[8px] text-[#475569]">{h.timeLabel}</span>
                    </div>
                  )
                })}
              </div>
              <div className="mt-3 flex items-center gap-4 text-[10px] text-[#475569]">
          ...
          {visibleObjects.length > 0 && (
            <div className="card p-5">
              <p className="text-xs text-[#64748B] uppercase tracking-widest mb-4 font-medium">{t('sky.planetsTitle')}</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {visibleObjects.map(obj => (
                  <div key={obj.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-[#475569]">{obj.maxAltitude}° · {obj.bestTime}</p>
              ...
              {planets.filter(p => !p.isVisible).length > 0 && (
                <div className="mt-3 pt-3 border-t border-white/[0.06]">
                  <p className="text-[10px] text-[#475569] mb-2">{t('sky.notVisible')}</p>
                  <div className="flex flex-wrap gap-2">
                    {planets.filter(p => !p.isVisible).map(obj => (
                      <span key={obj.id} className="text-[10px] text-[#475569] opacity-50">
